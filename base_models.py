...
dfin = pd.read_json(arg.data,compression='xz').sample(frac=1.0).reset_index()
...
for el in sets:
	for s in sets[el]:
		if s['atm'] not in headers:
			headers.append(s['atm'])
			atmpos.append(s['atm'])
...
for tr, te in kf.split(all_sets):
	train = [all_sets[i] for i in tr]
	test  = [all_sets[i] for i in te]
	model = dict()
	
	for s in train:
		if s['elm'] not in model: model[s['elm']] = list()
		model[s['elm']].append(s['cs'])
	...
	for aa in perf:
		...
		count = 0
		for ts in test:
			if ts['atm'] != aa: continue 
			count += 1
			rmse += (ts['cs'] - model[ts['elm']])**2
...
for aa in perf: perf[aa] /= pres[aa]

elmperf = ['element']
elmperf.extend(list(perf.values()))
performances.append(elmperf)
...
for aa in perf: 
	if pres[aa] == 0: continue
	perf[aa] /= pres[aa]

atmperf = ['atom']
atmperf.extend(list(perf.values()))
performances.append(atmperf)
...
for tr, te in kf.split(all_sets):
	...
	for aa in perf:
		...
		for ts in test:
			if ts['atm'] != aa: continue
			if ts['res'] not in model: continue
			if ts['atm'] not in model[ts['res']]: continue
			count += 1
...
atmresperf = ['atm-res']
atmresperf.extend(list(perf.values()))
performances.append(atmresperf)
...
for tr, te in kf.split(all_sets):
	...
	for aa in perf:
		...
		for ts in test:
			...
			if ts['atm'] not in model[key]: continue
			count += 1
...
merperf = ['3mer-atm']
merperf.extend(list(perf.values()))
performances.append(merperf)

writer = MarkdownTableWriter(
	table_name="Look-Up Table Model Results",
	headers=headers,
	value_matrix=performances,
	margin=1
)
